...
interface AddDoctorModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (doctor: { name: string; specialization: string; mobile: string; email: string }) => void
}
...
export default function AddDoctorModal({ isOpen, onClose, onSave }: AddDoctorModalProps) {
  ...
  const handleSave = () => {
    onSave({ name, specialization, mobile, email })
    setName("")
    setSpecialization("")
    setMobile("")
    setEmail("")
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-2xl rounded-sm overflow-hidden">
        ...
        <div className="bg-emerald-500 text-white px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-medium">Add Doctor Reference</h2>
